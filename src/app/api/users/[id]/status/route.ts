import { handleApiError, requireAdmin } from "@/lib/api/auth-guard";
import { errorResponse, successResponse } from "@/lib/response";
import { setUserStatus } from "@/lib/user-management";
import { toggleUserStatusSchema } from "@/schemas/user.schema";
import { NextRequest, NextResponse } from "next/server";

export async function PATCH(
  req: NextRequest,
  {
    params,
  }: {
    params: Promise<{ id: string }>;
  },
) {
  try {
    await requireAdmin();

    const parsed = toggleUserStatusSchema.safeParse(await req.json());

    if (!parsed.success) {
      return NextResponse.json(
        errorResponse(parsed.error.issues[0]?.message ?? "Validasi gagal", 400),
        { status: 400 },
      );
    }

    const user = await setUserStatus((await params).id, parsed.data.isActive);

    return NextResponse.json(
      successResponse(
        user,
        `Akun berhasil ${user.isActive ? "diaktifkan" : "dinonaktifkan"}`,
      ),
    );
  } catch (error) {
    return handleApiError(error, "PATCH /api/users/[id]/status");
  }
}
